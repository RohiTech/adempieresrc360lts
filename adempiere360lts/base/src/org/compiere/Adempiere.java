...
public final class Adempiere {
	...
	static {
		...
		InputStream inputStream = loader
				.getResourceAsStream("org/adempiere/version.properties");
		if (inputStream != null) {
			...
			try {
				...
				if (properties.containsKey("IMPLEMENTATION_VERSION"))
					s_ImplementationVersion = properties
							.getProperty("IMPLEMENTATION_VERSION");
				if (properties.containsKey("IMPLEMENTATION_VENDOR"))
					s_ImplementationVendor = properties
							.getProperty("IMPLEMENTATION_VENDOR");
			} catch (IOException e) {
			}
		}
	}
	...
	public static String getSummary() {
		StringBuffer sb = new StringBuffer();
		sb.append(NAME).append(" ").append(MAIN_VERSION).append("_")
				.append(DATE_VERSION).append(" -").append(SUB_TITLE)
				.append("- ").append(COPYRIGHT).append("; Implementation: ")
	...
	public static String getAdempiereHome() {
		...
		if (retValue == null
				&& System.getProperty("user.dir").indexOf("Adempiere") != -1) {
	...
	public static synchronized boolean startup(boolean isClient) {
		...
		if (isClient && Ini.isPropertyBool(Ini.P_TRACEFILE)
			...
			CLogMgt.addHandler(CLogFile.get(true, Ini.findAdempiereHome(),
					isClient));
	...
	public static boolean startupEnvironment(boolean isClient) {
		...
		try {
			...
			if (className == null || className.length() == 0) {
				className = System
						.getProperty(SecureInterface.ADEMPIERE_SECURE);
				if (className != null
						&& className.length() > 0
						&& !className
								.equals(SecureInterface.ADEMPIERE_SECURE_DEFAULT)) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println("ADempiere starting: " + className + " - "
					+ e.toString());
			e.printStackTrace();
		}
	} // main
